...
public class QuartzJobManager {
    ...
    public QuartzJobManager() {
        logger.info("初始化 jobUtil");
        jobUtil = this;
    }

    public static QuartzJobManager getInstance() {
        logger.info("返回  JobCreateUtil");
     ...
     * @param o
     */
//    public void addJob(Class clazz, String jobName, String jobGroupName, String cronExpression, Object o) {
...
//    }
     ...
     * @param jobGroupName   任务所在组名称
     * @param cronExpression cron表达式
     * @param jobDataMap         map形式参数
     */
    public void addJob(Class clazz, String jobName, String jobGroupName, String cronExpression, JobDataMap jobDataMap) {
        ...
        try {
            ...
            JobDetail jobDetail = JobBuilder
                                    .newJob(((Job) clazz.newInstance()).getClass())//创建新任务
                                    .withIdentity(jobName, jobGroupName).build();//将任务名称加入任务组
            //3.表达式调度构建器(即构建任务执行的时间)
            CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(cronExpression);
            //4.构建触发器:按新的cronExpression表达式构建一个新的trigger
            CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(jobName, jobGroupName).withSchedule(scheduleBuilder).build();
            //5.获得JobDataMap，写入数据
            if (jobDataMap != null) {
                trigger.getJobDataMap().put("jobDataMap", jobDataMap);
            }
            //6.调度器根据触发器调度任务
     ...
     * @param o
     */
    public void updateJob(String jobName, String jobGroupName, String cronExpression, Object o) {
